...
價格觸及掛單價就送出**限價單**掛在那個價位。截圖裡 17059 觸發、返點 10 → 掛 17049，
就是沒開跟隨時的樣子。`返點 = 0` 代表不等回檔，碰到觸發價當下就進場。
...
**送單價格不再往前穿價**：返點本身就是「等它回頭」的意思，穿過去等於放棄了回檔確認。
代價是掛單價沒成交就會停在 `sent`，由使用者決定要不要手動處理
...
**出場單的委託方式依目的而定**：停損／成本防線／收盤清倉一律**市價**——
這幾種都是「一定要出去」，掛限價沒成交就等於保護失效；停利則掛**限價**在停利價，
它是「有到價才要」，等不到就繼續持有沒關係。
...
- **收盤清倉**：到指定時間（`CONDITION_SESSION_CLOSE_TIMES`，預設日盤 13:44、夜盤 04:59）
  ...
  **未觸發的條件不會被刪除** —— 那是使用者辛苦設的，關掉總開關就達到「今天不再進場」的效果，
  ...
  真的連最後成交價都沒有（剛啟動就到點、該商品沒訂閱）時改送市價單 ——
  收盤前平不掉部位比滑價嚴重得多。
...
驗收標準：模擬帳戶設一個貼近市價的條件，觸發後在 `trade.log` 看到追價單送出、
