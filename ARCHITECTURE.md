...
三種停損（固定／保本／跟隨）不各自為政，由 `Condition.active_stop_price` 取**最保護**的那一個
（多單取 max、空單取 min）；能這樣直接合併是因為它們都只往有利方向動，不會互相拉扯。
出場時 `stop_kind` 會標出是被哪一種掃到的，`exit_reason` 因此有
`take_profit` / `stop_loss` / `cost_guard` / `trail` 四種。

跟隨距離沿用損點 —— **沒設損點就沒有距離可言**，保本與跟隨都不會啟動。

移動停損每個新高都會變，但**只有狀態變化（進 `guarded`）才寫 DB**；
純粹的停損位移只推播給前端。移動停損是盤中推算值，每個新高寫一次 SQLite
只是拿磁碟換沒人要的精度。
- **收盤清倉**：見 §7.6。
