...
- **新倉/平倉與已實現損益是自己推算的**（`core/fill_ledger.py`）：券商的成交回報只有買/賣，
  ...
  之後整天的新倉/平倉全部反過來。
  **`pnl` 與 `net_pnl` 是兩個數字**：`pnl` 一路都是「點位 × 點值」的毛損益（歷史上的意義
  沒變過）；`net_pnl` 才是扣完 `realized_fee`（手續費）與 `realized_tax`（交易稅）的最終損益，
  只有對到券商結算數字才算得出來——本地推算階段沒有真正的費用資料（成交回報的 `fee` 恆為 0，
  見 `sinopac.py`），硬扣只是拿假數字冒充精確值。前端顯示時優先用 `net_pnl`，對不到就退回
  `pnl` 並標星號（tooltip 講清楚是「本地推算，未扣手續費與交易稅」），今日損益合計也照這個
  優先序加總，不能每一列跟總和用不同的口徑。
...
- **成交語音提示**：前端收到 `fill` 就用瀏覽器的 `speechSynthesis` 念一句
  ...
  TTS 會把 TX 拆成兩個字母。
  **一定要合併再念**：一張市價單可能分成上百筆成交回報，而 `speechSynthesis`
  的佇列不會自己丟東西，逐筆念等於整場都在念剛才那張單。前端以第一筆成交起算
  一個固定 900ms 的窗口，同商品同方向同倉別的成交併成一句、價位取加權均價。
  窗口刻意**不隨後續成交延長** —— 延長的話，一串連續不斷的回報會讓它永遠等不到結束。
  狀態列有靜音開關（`localStorage`），關掉時一併 `cancel()` 佇列，
  否則按下靜音還會把已排入的唸完。
...
> **實作狀態**：**P1～P4 全部完成**，見 `core/condition_module.py`。分期內容見 §7.10。
...
| `pullback` | int | 返點 — 從極值回檔幾點才進場（0 = 碰到就進） |
| `qty` | int | 口數 |
| `take_profit` | int | 利點（0 = 不設停利） |
| `stop_loss` | int | 損點，UI 以負數輸入（0 = 不設停損） |
| `cost_guard` | bool | 成本防線 |
| `trail` | bool | 觸後跟隨 — 觸發後是否繼續追極值（進場端，見 §7.4） |
| `status` | enum | 見 §7.3 |
| `entry_order_id` / `entry_price` / `entry_filled_qty` | | 進場結果 |
| `exit_order_id` | | 出場委託 |
| `trigger_extreme` | float | 觸發後追到的極值（壓力空取最高、支撐多取最低） |
| `peak_price` | float | 成本防線用：進場後最有利價 |
...
```
主線
  waiting ─觸及觸發價─► triggered ─回檔到價送單─► sent ─成交回報─► filled ─停利/停損─► exited
...
進場是**兩段式**的。碰到觸發價只是「開始盯」，要等價格從極值**回檔「返點」**才真的送單 ——
壓力空在壓力區碰一下就放空常常是接刀，等它回頭才確認賣壓真的存在。

**第一段：觸發**（`waiting` → `triggered`）。方向跟現有觸價單**相反**，
右邊下單是在壓力/支撐**逆勢**接單：

| 條件 | 觸發判斷 | 意義 |
|------|---------|------|
| 壓力空 (`sell`) | `tick.price >= trigger_price` | 漲到壓力價 → 開始盯 |
| 支撐多 (`buy`) | `tick.price <= trigger_price` | 跌到支撐價 → 開始盯 |

**第二段：回檔進場**（`triggered` → `sent`）。以觸發後追到的極值
（`trigger_extreme`，壓力空取最高、支撐多取最低）為基準：

```
掛單價 entry_target_price = 極值 ∓ 返點
  壓力空：最高價 − 返點   （跌回來才空）
  支撐多：最低價 + 返點   （彈回來才多）
```

價格觸及掛單價就送出**一定範圍市價單（MWP + IOC）**。截圖裡 17059 觸發、返點 10 →
價格回到 17049 時送單，就是沒開跟隨時的樣子。`返點 = 0` 代表不等回檔，碰到觸發價當下就進場。

`entry_target_price` 決定的是**何時送單**，不是掛單價格 —— 回檔到價的判斷本模塊已經
在 tick 上做完了，送出去的當下價格就在進場價，再掛一張限價單只是多一次落空的機會。
範圍市價讓成交價被限制在保護範圍內，IOC 則讓撮不到的部分直接取消，不留半死不活的單在盤上。

**「觸後跟隨」控制的是極值會不會繼續走**：

| 跟隨 | 極值行為 | 掛單價 |
|------|---------|--------|
| Off | 凍結在觸發當下 | 固定 = `觸發價 ∓ 返點` |
| On | 續漲/續跌時一路推進（只進不退） | 跟著極值走，行情走多遠就抬多高 |

跟隨是**進場端**的機制，不是移動停損 —— 它決定「在哪裡進場」，
進場後的停損由固定停損與成本防線負責（見 §7.5）。

**不追價、不重送**：返點本身就是「等它回頭」的意思，穿過去等於放棄了回檔確認。
IOC 一口都沒吃到就停在 `failed`，由使用者決定要不要手動處理
（**不自動改價重送** —— 重掛迴圈在跳空時會一路追到底）。
吃到幾口就算進場成功，狀態進 `filled`、以 `entry_filled_qty` 管出場：
IOC 部分成交的回報是 `CANCELLED` 而不是 `FILLED`，當成失敗擱著的話，
真的建立起來的那幾口會完全沒有停利停損保護。
...
- **停利／停損**：`entry_price ± take_profit` / `∓ |stop_loss|`。兩者是 OCO，
  一邊觸及就送出場單並把條件推進 `exited`，另一邊失效 ——
  唯一的例外是停損可以搶掉還沒成交的停利單（見下方委託方式）。
- **成本防線**（`cost_guard`）：浮動獲利 ≥ `|stop_loss|` 時，把停損價移到 `entry_price`（保本），
  狀態轉 `guarded`。門檻用損點而非利點，是為了讓「賺到夠賠的量」就先立於不敗。
  判斷用的是 `peak_price`（看過的最大浮盈）而不是現價 —— 保本是棘輪，價格回落不該讓它失效。
> 注意：「觸後跟隨」是**進場端**的機制（§7.4），不是移動停損。進場後的停損只有
> 固定停損與成本防線兩種。

兩種停損不各自為政，由 `Condition.active_stop_price` 取**最保護**的那一個
（多單取 max、空單取 min）。出場時 `stop_kind` 會標出是被哪一種掃到的，
`exit_reason` 因此有 `take_profit` / `stop_loss` / `cost_guard` / `session_close` 四種。

保本門檻取自損點 —— **沒設損點就沒有門檻可言**，成本防線不會啟動。
- **收盤清倉**：見 §7.6。
...
| 出場原因 | 委託方式 | 為什麼 |
|---------|---------|--------|
| `take_profit` | **被動限價** LMT + ROD，掛在停利價 | 「有到價才要」，等不到就繼續持有，掛著也不吃價差 |
| `stop_loss` / `cost_guard` / `session_close` | **一定範圍市價** MWP + IOC | 「一定要出去」，掛限價沒成交就等於保護失效；範圍市價又擋掉流動性瞬間變差時被掃到離譜價位的成交 |

範圍市價與市價在期交所都**只能搭 IOC / FOK**，ROD 會被直接退件，
所以 `brokers/adapters/sinopac.py` 在送單前會把非法的 ROD 換成 IOC。

**保護性出場可以搶掉掛著的停利單**。停利是 ROD 限價單，「送出去」不等於「出得去」——
它可能一直掛在盤上不成交，而價格反向走到停損。這時若照 OCO 的「另一邊自然失效」擋下來，
整筆部位就沒有停損了。所以 `_may_send_exit` 允許保護性出場搶：先撤掉那張停利單
（撤單前已吃到的口數要記進已平量），再送範圍市價。反過來不成立，停利不會搶停利。

IOC 撮不完的部分會被取消，出場因此可能分成好幾張單：
`ConditionModule._exit_filled` 記錄已平口數，重送時只補剩下的
（不扣的話第二張單會照原口數再送一次，平完還會反手開出新倉），
`exit_price` 則按口數加權，不會被最後一張單的均價蓋掉。
部分成交算「有進展」，不計入 `MAX_EXIT_ATTEMPTS`——重試上限是用來擋
「怎麼送都出不去」的死循環，不該把一口一口慢慢平掉的大單也算成失敗。
...
- **可切換的商品**在 `settings.yaml` 的 `trading.symbols`（圖表下拉選單、下單面板的
  商品按鈕都讀它）。**開啟畫面時預設看哪一檔分兩處**：技術線圖讀
  `core_service.default_symbol`（關乎「Core 開機盯哪一檔」，最好也列進
  `default_subscribe_symbols`），下單面板讀 `trading.default_symbol`；兩者可以不同檔。
  任一預設商品不在 `trading.symbols` 裡會在啟動時直接報錯 —— 選不到的預設等於沒設，
  而畫面上只會表現成「圖表沒資料」，完全看不出是設定檔的問題。
- **新條件的預設值**（返點／口數／利點／損點／成本防線／觸後跟隨）在
  `settings.yaml` 的 `condition.defaults`，經 `/api/config` 給前端。
  壓力價／支撐價**不給預設** —— 那是每筆都不同的東西，給了只會變成「忘了改就送出去」。
  「啟動交易」也**刻意不可設定**：開機就自動把昨天留下的條件送進市場不該是個選項。
  當沖／收盤清倉這兩個引擎旗標沒有持久化，每次重啟一樣從這裡讀。
- **當沖**：勾選後會**自動把「收盤清倉」一起打開**（當沖部位留倉就不是當沖了，
  兩個旗標各走各的只會製造「以為在當沖、實際留倉」的意外）；反之關掉收盤清倉也會清掉當沖旗標。
  下單的新倉/平倉別本來就固定是進場 `octype="new"`、出場 `octype="cover"`，
  這個旗標**不會**額外送出券商層的當沖保證金註記。
- **收盤清倉**：到指定時間（`CONDITION_SESSION_CLOSE_TIMES`，預設日盤 13:44、夜盤 04:59）
  平掉本引擎建立的部位，並把交易總開關切回暫停。只清本引擎自己的部位，不碰手動下的單。
  **未觸發的條件不會被刪除** —— 那是使用者辛苦設的，關掉總開關就達到「今天不再進場」的效果，
  明天要不要繼續由使用者決定。時間點抓在收盤前一分鐘，留一分鐘讓平倉單有機會成交。
  判斷方式是「**上次檢查 → 現在**這段區間有沒有跨過收盤時點」，不是「現在是不是剛好那一分鐘」：
  比對當下分鐘的話，筆電從 13:30 睡到 15:00 就會讓 13:44 從來沒被看到，部位直接留倉過夜。
  用區間也順帶擋掉另一頭 —— 程式在晚上才啟動時沒有「上一次檢查」，不會補跑白天的清倉。
  沒有最後成交價（剛啟動就到點、該商品沒訂閱）也照樣送單 —— 收盤清倉走範圍市價，
  本來就不需要價格，收盤前平不掉部位比滑價嚴重得多。
...
tick (EventBus)
  ...
  → 觸發 → 等回檔 → TradeModule.place_order()   # 共用同一個下單入口
...
`delete_condition`、`set_condition_trading`、`set_condition_options`（當沖／收盤清倉）。
前端改為**不再用 localStorage 存條件**，一律以後端推來的清單為準（多分頁才會一致）；
三個全域開關的真相同樣在後端，任一分頁切換都會廣播給其他分頁。
...
處理方式分兩段，中間那段時間條件完全不動：

1. `load_from_db()`（`setup()` 內，此時還沒連券商）—— 所有進行中的條件**一律先標成 `orphaned`**。
   `orphaned` 不在 `is_holding` 裡，所以就算 tick 進來也不會觸發任何送單。
2. `reconcile_with_broker(positions)`（`startup_core` 連上券商之後）—— 拿真實倉位比對，
   對得上的才恢復成 `filled` 繼續管理。

比對用**每個商品的淨口數**，不試圖把條件一對一配到某張券商委託上 ——
券商倉位是彙總後的數字，本來就分不出哪一口屬於哪個條件。判定規則：

- 同商品條件的預期淨口數與券商實際部位**方向一致、且實際口數不少於預期** → 整組恢復。
  券商端多出來的口數可能是手動下的單，不歸引擎管，不該擋住接手。
- 對不起來 → 該商品的條件**整組**留在 `orphaned`，不猜是哪一筆出錯。
- 重啟前停在 `sent`（進場單還在路上）的條件**永遠不恢復**：那張單成交了沒、成交幾口，
  重啟後已經無從得知，猜錯就是拿錯誤的成本去掛停損。

`orphaned` 的條件會在前端列表顯示為「待確認」並跳出提示，等使用者確認部位後自行處理或刪除。
...
| **P3** | 成本防線（觸後跟隨併入 §7.4 進場端） | + guarded | ✅ 已完成 |
| **P4** | 收盤清倉 + 當沖旗標 + 重啟對帳 | + orphaned | ✅ 已完成 |

驗收標準：模擬帳戶設一個貼近市價的條件，觸發後在 `trade.log` 看到進場單送出、
成交後條件停在 `filled`，價格再走到停利／停損時看到 `octype=cover` 的平倉單，
且**同一個條件的進場與出場各只送出一張單**；重啟 server 後，
持倉中的條件應在連上券商後自動恢復成 `filled`（log 會印「對帳相符」）。
